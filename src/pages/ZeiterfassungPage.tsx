...
import AufgabePickerDropdown from '../components/AufgabePickerDropdown';
...
function formatDauer(sek: number) {
...
export default function ZeiterfassungPage() {
  ...
  const [aktiv, setAktiv] = useState<Aufgabe | null>(null);
  ...
  function handleStop() {
    ...
    setLaeuft(false);
    if (!aktiv || !startRef.current) return;

    const endzeit = new Date();
    const dauerSekunden = Math.round((endzeit.getTime() - startRef.current.getTime()) / 1000);
    const eintrag = speichereZeiteintrag({
      aufgabeId: aktiv.id,
      aufgabeTitel: aktiv.titel,
      aufgabeProjekt: aktiv.projekt,
    ...
    const min = Math.round(dauerSekunden / 60);
    setToast({ visible: true, message: `${min} Min. für "${aktiv.titel}" erfasst ✓` });
  }

  const hideToast = useCallback(() => setToast((t) => ({ ...t, visible: false })), []);
  const gesamtSek = eintraege.reduce((s, e) => s + e.dauerSekunden, 0);
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="px-4 pt-4">
        <AufgabePickerDropdown
          aufgaben={aufgaben}
          ausgewaehlt={aktiv}
          onAuswaehlen={setAktiv}
          disabled={laeuft}
        />
      </div>
      ...
      <div className="flex flex-col items-center py-10 gap-3">
        {aktiv?.projekt && (
          <p className="text-xs font-bold text-brand uppercase tracking-widest">{aktiv.projekt}</p>
        )}
        <p className={`tabular text-7xl font-thin tracking-widest ${laeuft ? 'text-gray-900' : 'text-gray-300'}`}>
          {formatZeit(sekunden)}
        </p>
        {laeuft && (
      ...
      <div className="flex justify-center pb-8">
        <button
          ...
          className={`flex items-center gap-3 px-10 py-4 rounded-full text-white text-lg font-bold shadow-lg transition-all active:scale-95 ${
            laeuft ? 'bg-red-500 shadow-red-500/30'
            : aktiv ? 'bg-brand shadow-brand/30'
            : 'bg-gray-300 cursor-not-allowed shadow-none'
          }`}
        >
          {laeuft
            ? <><Square size={20} fill="white" /> Stopp</>
      ...
      <div className="flex-1 px-4 pb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          {gesamtSek > 0 && (
            <span className="text-xs font-bold text-brand">{formatDauer(gesamtSek)} gesamt</span>
          )}
        </div>
